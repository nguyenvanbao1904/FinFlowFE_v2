...
public class WelcomeBackViewModel {
    ...
    private let biometricHandler = SessionBiometricAuthCoordinator()
    ...
    public func authenticateWithPIN() async {
        ...
        do {
            ...
            if let appError = error as? AppError, case .unauthorized = appError {
                alert = .authWithAction(message: AppErrorAlert.sessionExpiredMessage) { [weak self] in
                    Task { @MainActor in
            ...
            Logger.warning(
                "PIN failed: allowed=\(result.allowed), attempts=\(result.attempts)/\(result.max)",
                category: "Security")
            if result.allowed {
                alert = .auth(
                    message:
    ...
    public func loginWithBiometric() async {
        ...
        let alertResult = await biometricHandler.authenticate(
            ...
            options: SessionBiometricAuthCoordinator.Preset.welcomeBack
        )
